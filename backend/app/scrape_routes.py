...
def clean_caption(caption):
    # If caption is a list, join it into a single string
    if isinstance(caption, list):
        caption = " ".join(caption)
    # Remove {" "} and any surrounding whitespace from the caption
    cleaned_caption = caption.replace('{" "}', '').strip()
    return cleaned_caption



# Function to parse a post from Instagram's data
def parse_post(data: Dict) -> Dict:
    ...
    if result:
        ...
        result['captions'] = clean_caption(result['captions'])
        return result
    else:
...
# Function to scrape posts and add to database
def scrape_posts(insta_account_id, insta_username):
    if not insta_username:
        return {"error": "Invalid Instagram username"}, 404

    new_posts_count = 0
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            for post in scrape_user_posts(insta_account_id, max_pages=3):
                # Check if the post already exists in the database
                cur.execute("SELECT 1 FROM posts WHERE post_id_str = %s", (post['id'],))
                if cur.fetchone():
                ...
                cur.execute("""
                    INSERT INTO posts (
                        insta_account_id, post_id_str, shortcode, dimensions_height, dimensions_width,
                        src, src_attached, video_url, location, taken_at, is_video, link, caption_text
                    )
                    VALUES (
                ...
                """, (
                    ...
                    post['src'],
                    post['src_attached'][0] if post['src_attached'] else None,
                    post['video_url'],
                    post['location'],
                    post['taken_at'],
                    post['is_video'],
    ...
    return {"message": f"Scraped {new_posts_count} new posts for {insta_account_id}"}, 200
